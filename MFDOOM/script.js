document.addEventListener("DOMContentLoaded", () => {
    ...
    const frases = [
      '"Remember ALL CAPS when you spell the man name" - All Caps',
      '"Living off borrowed time, the clock tick faster" - Accordion',
      '"Got more soul than a sock with a hole" - Doomsday',
      "\"I wear a mask so when I face you face-to-face, you can't tell if I'm lying\" - MF DOOM",
      '"Villain get the money like curls, they just tryin\' to get a nut like squirrels in his mad world" - Curls',
      "\"On Doomsday, ever since the womb 'til I'm back where my brother went\" - Doomsday",
      '"The rest is empty with no brain but the clever nerd" - Figaro',
      '"The supervillain cooler than a million, I be chillin\'" - All Caps',
      '"Last wish: I wish I had two more wishes" - Great Day',
      '"The mask is like Jason, they told the place not to let the basket type case in" - Meat Grinder',
    ]
    ...
    function animateStats() {
      statNumbers.forEach((stat) => {
        ...
        const updateCount = () => {
          ...
          stat.textContent = stat.textContent.includes("M+") ? `${currentCount}M+` : `${currentCount}${suffix}`
  
          if (progress < 1) {
            requestAnimationFrame(updateCount)
          } else {
            stat.textContent = stat.textContent.includes("M+") ? `${target}M+` : `${target}${suffix}`
          }
        }
    ...
    // Funcionalidad para el análisis lírico
    ...
    const legadoCulturalItems = document.querySelectorAll(".legado-cultural-item")
    ...
    if (sobreCards.length > 0) {
      sobreCards.forEach((card) => {
        card.addEventListener("mouseenter", function () {
          this.querySelector(".sobre-card-inner").style.transform = "rotateY(180deg)"
        })
  
        card.addEventListener("mouseleave", function () {
          this.querySelector(".sobre-card-inner").style.transform = "rotateY(0deg)"
        })
      })
    }
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              const finalValue = Number.parseInt(entry.target.textContent.replace(/[^0-9]/g, ""))
    ...
    function handleRotation(direction) {
        ...
        isAnimating = true;
        const className = direction === 'left' ? 'rotate-left-anim' : 'rotate-right-anim';
    ...
    function updateMaskVisibility() {
        maskItems.forEach((item, index) => {
            if (index === currentIndex) {
                ...
                item.style.transform = `translateX(${index < currentIndex ? '-100%' : '100%'})`;
